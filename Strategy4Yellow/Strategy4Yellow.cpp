...
{	
	...
	case 6:
		...
		laterRobots[2].pos.y = 80;
		laterRobots[2].rotation = 0;
		laterRobots[3].pos.x = 70;
		laterRobots[3].pos.y = 90;
		laterRobots[3].rotation = 0;
		laterRobots[4].pos.x = 70;
		laterRobots[4].pos.y = 100;
		laterRobots[4].rotation = 0;
		break;	
	case 7:
		laterRobots[0].pos.x = 1;
		...
		laterRobots[2].pos.y = 150;
		laterRobots[2].rotation = 0;
		laterRobots[3].pos.x = 182;
		laterRobots[3].pos.y = 20;
		laterRobots[3].rotation = 270;
...
STRATEGY4YELLOW_API void RunStrategy(Environment *pEnv)
{
	...
	Goalie(pEnv);
	CenterDefender(pEnv,1);
	Jish_Attack(&pEnv->home[2], pEnv);
	Jish_Support(&pEnv->home[3], pEnv);
	Jish_Attack(&pEnv->home[4], pEnv);
	/*LeftWing(pEnv, 1);
	RightWing(pEnv, 2);
...
void Goalie(Environment* pEnv)
{
	...
	{
		Position2(pEnv, 0, 1, 90);
	}
	else
	{
		if (pEnv->currentBall.pos.y > 115)
		{
			Position2(pEnv, 0, 1, 110);
		}
		else if (pEnv->currentBall.pos.y < 65)
		{
			Position2(pEnv, 0, 1, 70);
		}
		else
		{
			Position2(pEnv, 0, 1, pEnv->currentBall.pos.y);
